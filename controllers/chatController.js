...
const createChat = async (req, res) => {
  try {
    ...
    // For group chats, check if a group chat with the same connectionId already exists
    if (type === 'group') {
      let existingGroupChat = null;
      
      // First check by connectionId if available
      if (metadata?.connectionId) {
        existingGroupChat = await Chat.findOne({
          type: 'group',
          'metadata.connectionId': metadata.connectionId
        }).populate('participants.userId', 'name username');
        
        console.log('Create chat - Checking by connectionId:', metadata.connectionId);
        console.log('Create chat - Existing group chat by connectionId:', existingGroupChat);
      }
      
      // If no chat found by connectionId, check by participants (fallback)
      if (!existingGroupChat) {
        ...
        console.log('Create chat - Checking by participants (fallback)');
        console.log('Create chat - Existing group chat by participants:', existingGroupChat);
      }
    ...
    const chat = new Chat({
      ...
      metadata: {
        ...
        connectionId: type === 'group' ? metadata?.connectionId : undefined,
...
const getMessages = async (req, res) => {
  try {
    ...
    const { page = 1, limit = 50, connectionId } = req.query;
    const userId = req.user.id;

    console.log('Fetching messages for chat:', chatId, 'connectionId:', connectionId);
    ...
    // If connectionId is provided and it's a group chat, use filtered messages
    if (connectionId && chat.type === 'group') {
      const connectionUpdateService = require('../services/connectionUpdateService');
      messages = await connectionUpdateService.getFilteredMessages(chatId, userId, connectionId, page, limit);
    } else {
      // Get all messages for private chats or when no connectionId
...
const sendMessage = async (req, res) => {
  try {
    ...
    if (io) {
      ...
      console.log('[SOCKET_DEBUG] Emitting to room: chat:${chatId}');
      ...
      // Debug: Check room members before emitting
      try {
        const roomMembers = await io.in(`chat:${chatId}`).fetchSockets();
        console.log(`[SOCKET_DEBUG] Room chat:${chatId} has ${roomMembers.length} members before emitting`);
        roomMembers.forEach(member => {
          console.log(`[SOCKET_DEBUG] Room member: ${member.user?.id || 'unknown'}`);
        });
      } catch (error) {
        console.log('[SOCKET_DEBUG] Error checking room members:', error.message);
      }
